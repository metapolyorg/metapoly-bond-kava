...
let DVDAddr //= "0x77dcE26c03a9B833fc2D7C31C22Da4f42e9d9582"
let USDCAddr //= "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"
let WROSEAddr //= "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"
...
describe("USM", () => {
    it("Should work", async () => {
        ...
        const DVDFac = await ethers.getContractFactory("TestERC20", deployer)
        const DVD = await DVDFac.deploy("DVD", "DVD", "18")
        await DVD.mint(ethers.utils.parseUnits("21000", "18"))
        DVDAddr = DVD.address
        // await router.swapETHForExactTokens(
        //     ethers.utils.parseEther("1000"), [WROSEAddr, DVDAddr], deployer.address, Math.ceil(Date.now() / 1000) + 31539000,
        ...
        const USDC = await USDCFac.deploy("DVD", "DVD", "6")
        await USDC.mint(ethers.utils.parseUnits("21000", "6"))
        USDCAddr = USDC.address

        const WROSEFac = await ethers.getContractFactory("TestERC20", deployer)
        const WROSE = await WROSEFac.deploy("WROSE", "WROSE", "18")
        await WROSE.mint(ethers.utils.parseUnits("21000", "18"))
        WROSEAddr = WROSE.address

        // Deploy D33D
        const D33DFac = await ethers.getContractFactory("Token", deployer)
        const D33D = await upgrades.deployProxy(D33DFac, ["D33D", "D33D", 18])

        const mockRouter = await smock.fake(routerABI, { provider: deployer })
        await mockRouter.getAmountsOut.returns((args) => {
            if (args.path[0] == D33D.address) {
                return [args.amountIn, args.amountIn.mul(ethers.utils.parseUnits("0.8", "6")).div(ethers.utils.parseEther("1"))]
            } else {
                return [args.amountIn, 0, args.amountIn.mul(ethers.utils.parseUnits("0.1", "6")).div(ethers.utils.parseEther("1"))]
            }
        })
